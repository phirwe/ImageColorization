...
X = []
for filename in os.listdir('Dataset/face_color_256/'):
        ext = filename[filename.index(filename[:-3][-1])+1:]
        print ('here')
        img = Image.open('Dataset/face_color_256/'+filename)
        #img = img.resize((256, 256), PIL.Image.ANTIALIAS)
        #img.save('Train/'+filename)
        X.append(img_to_array(load_img('Dataset/face_color_256/'+filename)))
...
filenames = []
color_me = []
for filename in os.listdir('Dataset/face_test_256/'):
    filenames.append(filename)
    color_me.append(img_to_array(load_img('Dataset/face_test_256/'+filename)))
...
for i in range(len(output)):
    ...
    cur[:,:,0] = color_me[i][:,:,0]
    cur[:,:,1:] = output[i]
    imsave("1000Epochs_face/April21_"+filenames[i], lab2rgb(cur))
